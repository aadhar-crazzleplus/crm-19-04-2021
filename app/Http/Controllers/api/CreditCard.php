...
class CreditCard extends BaseController
{
    ...
    public function addcard_profile(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'full_name' => 'required',
            'mobile_no' => 'required|min:10|max:10',
        ...
        $is_card = lead_is_cards::where("lead_profile_id",$lead_pro->id)->get();
        $card_arr=array();
        if(!empty($is_card)){
            foreach($is_card as $value){
                $card_arr[] = [
                    'card_id' => $value->id,
                    'bank_id' => $value->bank_id,
                    'total_limit' => $value->total_limit,
                    'ava_limit' => $value->ava_limit,
                    'card_vintage' => $value->card_vintage
                ];
            }
            // $card_arr["card_id"]=$is_card->id;
            // $card_arr["bank_id"]=$is_card->bank_id;
            // $card_arr["total_limit"]=$is_card->total_limit;
            // $card_arr["ava_limit"]=$is_card->ava_limit;
            // $card_arr["card_vintage"]=$is_card->card_vintage;
        }

        $data["profile_id"] = $lead_pro->id;
        $data["lead_id"] = $lead->id;
        $data["is_card"] = !empty($card_arr)?$card_arr:NULL;
        return $this->sendResponse(new ProjectResource($data), "Performed successfully");
    }

    public function addcard_is_card(Request $request)
    {
        ...
        $card_json = json_encode($request->card_arr);
        $card_arr = json_decode($card_json,true);
        ...
        if(!empty($card_arr)){
            foreach($card_arr as $value){
                if(isset($value["card_id"]) && $value["card_id"] > 0){
                    $is_card = lead_is_cards::find($value["card_id"]);
                    if(!empty($is_card) && $is_card->bank_id != ""){
                        $is_card->bank_id = $value["bank_id"];
                        $is_card->total_limit = $value["total_limit"];
                        $is_card->ava_limit = $value["ava_limit"];
                        $is_card->card_vintage = $value["card_vintage"];
                        $is_card->update();
                    }else{
                        return $this->sendError('Card Validation.', array("err_msg"=>["Card Id does not exist !!"]));
                    }

                }else{
                    $is_card = new lead_is_cards();
                    $is_card->lead_profile_id = $request->profile_id;
                    $is_card->bank_id = $value["bank_id"];
                    $is_card->total_limit = $value["total_limit"];
                    $is_card->ava_limit = $value["ava_limit"];
                    $is_card->card_vintage = $value["card_vintage"];
        ...
        $arr_inocome=array();
        if(!empty($res_arr) && $res_arr->occupation_id == "1"){
            $arr_inocome["occupation_id"]=$res_arr->occupation_id;
            $arr_inocome["occu_title"]=$res_arr->occupation->occu_title;
        ...
        }elseif(!empty($res_arr) && $res_arr->occupation_id == "2"){
            $arr_inocome["occupation_id"]=$res_arr->occupation_id;
            $arr_inocome["occu_title"]=$res_arr->occupation->occu_title;
            $arr_inocome["itr_amount"]=$res_arr->itr_amount;
            $arr_inocome["gst_no"]=$res_arr->gst_no;
            $arr_inocome["gst_vintage"]=$res_arr->gst_vintage;
            $arr_inocome["company_id"]=$res_arr->company_id;
            $arr_inocome["company_name"]=$res_arr->company_name;
            $arr_inocome["office_email"]=$res_arr->office_email;
            $arr_inocome["company_address"]=$res_arr->company->address;
            $arr_inocome["pincode_id"]=$res_arr->company->pincode_id;
            $arr_inocome["city_id"]=$res_arr->company->city_id;
        ...
        // $data["incomess"] = $request->card_arr;

        return $this->sendResponse(new ProjectResource($data), "Performed successfully");
    }

    public function addcard_income(Request $request)
    {
        ...
        if($request->occupation_id == "1"){
            ...
            $res_arr->company_id=$company_id;
            $res_arr->gst_no=$request->gst_no;
            $res_arr->gst_vintage=$request->gst_vintage;
            $res_arr->office_email=$request->office_email;
            $res_arr->update();
        }else{
    ...
    {
        if(isset($request->occupation_id) && $request->occupation_id != ""){
            $occupation = occupation::where("id",$request->occupation_id)->get(['id','occu_title']);
    ...
    public function destroy(Request $request)
    {
        ...
        lead_is_cards::where('id', $request->card_id)->delete();
